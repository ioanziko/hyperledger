...
DEFAULT_SERVER_CONFIG_YAML="/etc/hyperledger/config/ica/fabric-ca-server-config.yaml"
DEFAULT_CLIENT_CONFIG_YAML="/etc/hyperledger/config/ica/admin/fabric-ca-client-config.yaml"
...
enroll() {
    ...
    export FABRIC_CA_CLIENT_HOME=/etc/hyperledger/client/caserver/admin
    ...
    fabric-ca-client enroll -u https://admin:$PASSWORD@$CA_ADDRESS:7054 --tls.certfiles /etc/hyperledger/server/ca-cert.pem --enrollment.profile tls
}
...
start() {
    ...
    then   
        ...
        cp $DEFAULT_SERVER_CONFIG_YAML  /etc/hyperledger/server
    fi
    ...
    #sleep 1s
    #mkdir -p /etc/hyperledger/server/msp/signcerts
    #cp /etc/hyperledger/rootclient/ica/admin/msp/tls/cert.pem /etc/hyperledger/server/msp/signcerts/cert.pem
}
